...
public class CookieManager {
    ...
    public boolean saveCookies(RecruitmentPlatformEnum platform, Page page) {
        try {
            ...
            // 注意：不使用 page.waitForLoadState/waitForTimeout，因为这会与主线程的导航操作冲突导致Playwright状态损坏
            try {
                page.waitForLoadState(LoadState.NETWORKIDLE);
            } catch (Exception ignored) {
                // 忽略：页面可能正在导航中，无法等待NETWORKIDLE
            }

            // 额外等待一小段时间，确保JavaScript有足够时间处理并写入Cookie
            // 使用 Thread.sleep 代替 page.waitForTimeout，避免与主线程的Playwright操作冲突
            Thread.sleep(500);
