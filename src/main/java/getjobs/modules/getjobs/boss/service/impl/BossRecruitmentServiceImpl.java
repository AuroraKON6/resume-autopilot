            ...
            for (int i = 0; i < itemCount; i++) {
                ...
                try {
                    ...
                    if (companyName != null && message != null) {
                        ...
                        if (match && !nomatch) {
                            // 从数据库查询获取黑名单公司列表
                            final String currentCompanyName = companyName;
                            List<String> existingBlackCompanies = userProfileRepository.findAll().stream()
                                    .findFirst()
                                    .map(profile -> profile.getCompanyBlacklist())
                                    .orElse(Collections.emptyList());

                            // 检查公司是否已在黑名单中
                            boolean alreadyInBlacklist = existingBlackCompanies.stream()
                                    .anyMatch(blackCompany -> currentCompanyName.contains(blackCompany));

                            if (!alreadyInBlacklist) {
                                // 清理公司名称
                                String cleanedCompanyName = companyName.replaceAll("\\.{3}", "");
                                // 验证公司名称格式（至少2个中文字符或4个英文字符）
                                if (cleanedCompanyName.matches(".*(\\p{IsHan}{2,}|[a-zA-Z]{4,}).*")) {
                                    // 添加到数据库黑名单
                                    try {
                                        ...
                                        if (userProfile != null) {
                                            List<String> companyBlacklist = userProfile.getCompanyBlacklist();
                                            if (companyBlacklist == null) {
                                                companyBlacklist = new ArrayList<>();
                                            }
                                            companyBlacklist.add(cleanedCompanyName);
                                            userProfile.setCompanyBlacklist(companyBlacklist);
                                            userProfileRepository.save(userProfile);

                                            newBlackCompanies++;
                                            log.debug("新增黑名单公司：{} - 拒绝信息：{}", cleanedCompanyName, message);
                                        }
                                    } catch (Exception e) {
                                        log.error("保存黑名单公司失败: {}", cleanedCompanyName, e);
                                    }
                                }
                            }
                        }
